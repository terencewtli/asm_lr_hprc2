...
summaries), R941 donors only so chemistry doesn't blur the gradient (R1041 runs ~4 points lower,
mostly inside PMDs). Tracks are ordered and coloured from lowest to highest global mCG.
...
longLabel UCLA Luo lab - HPRC2 LCL hap1 ONT CpG methylation for 20 donors ordered by global mCG
...
def select() -> pd.DataFrame:
    ...
    s = s[s['sample'].map(chem) == CHEM].sort_values('global_wmeth').reset_index(drop=True)
    targets = np.linspace(s.global_wmeth.min(), s.global_wmeth.max(), N_DONORS)
    picked = []
    for t in targets:
...
longLabel hap1 per-CpG mCG, 20 R941 donors ordered from lowest (top) to highest (bottom) global mCG
centerLabel off
""")
    missing = []
    for r in sel.itertuples():
        ...
        if not src.exists():
            ...
            continue
        dst = G / f'{r.rank:02d}_{r.sample}_hap1_mCG.bw'
  ...
  longLabel #{r.rank} {r.sample} hap1 | {r.superpopulation} | global mCG {r.global_wmeth:.3f} | in/out own PMDs {r.meth_in_pmd_native:.2f}/{r.meth_out_pmd_native:.2f}
  priority {r.rank}
  color {color}
  visibility full
  windowingFunction mean
